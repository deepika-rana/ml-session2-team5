...
class WebScraper:
    ...
    def get_post_title_details(self, topic_soup):
        """
        Get post title, category and tags
        """
        post_title = topic_soup.find('a', class_='fancy-title').text.strip()
        ...
        return post_title, topic_category, topic_tags
    ...
    def get_topic_comments(self, topic_soup):
        ...
        postsDivs = postStream.find_all('div', {'class': [
                                        'topic-post clearfix topic-owner regular', 'topic-post clearfix regular']})
    ...
    def get_topic_users_nbr(self, topic_soup):
        """
        Get the topic's nbr of users
        """
        users = topic_soup.find('li', class_="secondary users")

        if users is None:
            nbr_users = str(0)
        else:
            nbr_users = users.find('span', class_='number').text

        return nbr_users

    def get_topic_links_nbr(self, topic_soup):
        """
        Get the topic's nbr of links
        """
        links = topic_soup.find('li', class_="secondary links")

        if links is None:
            nbr_links = str(0)
        else:
            nbr_links = links.find('span', class_='number').text
    ...
    def runApp(self, BASE_URL, SITE_NAME):
        ...
        for categ_url in categ_urls:
            ...
            categ_topic_links = categ_topic_soup.find_all(
            ...
            for categ_topic_url in categ_topic_urls:
                ...
                topic_title, topic_category, topic_tags = self.get_post_title_details(
                    topic_soup)
                leading_comment, other_comments = self.get_topic_comments(
                    topic_soup)
                ...
                nbr_users = self.get_topic_users_nbr(topic_soup)
                nbr_links = self.get_topic_links_nbr(topic_soup)
                ...
                self.topic_df = self.topic_df.append(
                    attribute_dict, ignore_index=True)
...
if __name__ == '__main__':
    ...
    webdriverPath = r'C:\Users\dutta\Downloads\geckodriver-v0.29.1-win64\geckodriver'
